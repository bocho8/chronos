...
try {
    ...
    // Get docente ID for current user
    $docenteQuery = "SELECT d.id_docente, u.nombre, u.apellido, u.email, u.cedula
                     FROM docente d
                     JOIN usuario u ON d.id_usuario = u.id_usuario
                     WHERE d.id_usuario = :id_usuario";
    $docenteStmt = $db->prepare($docenteQuery);
    $docenteStmt->bindParam(':id_usuario', $userId, PDO::PARAM_INT);
    $docenteStmt->execute();
    $selectedDocente = $docenteStmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$selectedDocente) {
        throw new Exception("Docente no encontrado");
    }
    
    $selectedDocenteId = $selectedDocente['id_docente'];
    ...
    // Load disponibilidad for current teacher
...
<body class="bg-bg font-sans text-gray-800 leading-relaxed">
            ...
            <section class="flex-1 p-3 sm:p-4 md:p-6 w-full overflow-x-hidden">
                <div class="max-w-6xl mx-auto">
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-lightborder p-12 text-center">
                        ...
                        <p class="text-gray-500"><?php echo htmlspecialchars($error_message ?? 'No se pudo cargar la información del docente'); ?></p>
                    </div>
                    <?php endif; ?>
                </div>
            </section>
    ...
    <script>
        function toggleDisponibilidad(cell) {
            ...
            fetch('/api/teacher/disponibilidad', {
                method: 'POST',
                body: formData
            })
        ...
        function saveObservacion() {
            ...
            fetch('/api/teacher/observaciones', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
        ...
        function editObservacion(id) {
            fetch('/api/teacher/observaciones?action=get_observacion&id_observacion=' + id)
                .then(response => response.json())
                .then(data => {
